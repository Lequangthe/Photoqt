...
import dev.leonlatsch.photoqt.ui.theme.Colors
import kotlinx.coroutines.launch

sealed interface ImportChoice {
    data class AddNewFiles(val fileUris: List<Uri>) : ImportChoice
    data class AddNewFilesWithAlbum(val fileUris: List<Uri>, val albumUuid: String) : ImportChoice
...
private fun ImportMenuDialogContent(
    ...
    var pendingFileUris by remember { mutableStateOf<List<Uri>?>(null) }
    ...
    pendingFileUris?.let { uris ->
        AlbumPickerDialog(
            ...
            onDismissRequest = { pendingFileUris = null },
            onAlbumSelected = { albumUuid ->
                onImportChoice(
                    ImportChoice.AddNewFilesWithAlbum(fileUris = uris, albumUuid = albumUuid)
                )
    ...
    Column(
        ...
        ImportMenuItem(
            text = stringResource(R.string.import_menu_browse_files_title),
            description = stringResource(R.string.import_menu_browse_files_description),
            iconPainter = painterResource(R.drawable.ic_folder),
            onClick = {
                browseFilesLauncher.launchAndIgnoreTimer(
                    input = arrayOf("*/*"),
                    activity = activity,
                )
            }
