...
package dev.leonlatsch.photoqt.io
...
class IO @Inject constructor(
    ...
    fun deleteFile(fileUri: Uri): Boolean? {
        Timber.d("deleteFile: attempting to delete %s", fileUri)
        return try {
            val rowsDeleted = context.contentResolver.delete(fileUri, null, null)
            Timber.d("deleteFile: ContentResolver returned rowsDeleted=%s", rowsDeleted)
            if (rowsDeleted > 0) return true

            try {
                DocumentsContract.deleteDocument(context.contentResolver, fileUri)
                Timber.d("deleteFile: DocumentsContract.deleteDocument succeeded")
                return true
            } catch (e: FileNotFoundException) {
                Timber.d("deleteFile: DocumentsContract not found: %s", e.message)
            } catch (e: UnsupportedOperationException) {
                Timber.d("deleteFile: DocumentsContract not supported: %s", e.message)
            }

            if (fileUri.authority == "com.android.providers.media.documents") {
                try {
                    ...
                    if (parts.size >= 2) {
                        val type = parts[0]
                        val id = parts.last()
                        val tableUri = when (type) {
                            "image" -> MediaStore.Images.Media.EXTERNAL_CONTENT_URI
                            "video" -> MediaStore.Video.Media.EXTERNAL_CONTENT_URI
                            "audio" -> MediaStore.Audio.Media.EXTERNAL_CONTENT_URI
                            else -> Uri.parse("content://media/external/${type}s/media")
                        }
                        val selection = "${MediaStore.MediaColumns._ID} = ?"
                        val selArgs = arrayOf(id)
                        val mediaRows = context.contentResolver.delete(tableUri, selection, selArgs)
                        Timber.d("deleteFile: MediaStore fallback rowsDeleted=%s", mediaRows)
                        if (mediaRows > 0) return true
                    }
                } catch (e: Exception) {
                    Timber.d("deleteFile: MediaStore fallback error: %s", e.message)
                }
            }

            val srcDoc = DocumentFile.fromSingleUri(context, fileUri)
            val docResult = srcDoc?.delete()
            Timber.d("deleteFile: DocumentFile delete returned %s", docResult)
            if (docResult == true) return true

            Timber.d("deleteFile: all methods failed")
            null
        } catch (e: Exception) {
