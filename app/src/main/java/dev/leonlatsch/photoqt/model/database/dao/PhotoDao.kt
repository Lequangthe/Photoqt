package dev.leonlatsch.photoqt.model.database.dao
...
interface PhotoDao {
    ...
    fun observeRecentlyAddedSorted(since: Long, sort: Sort): Flow<List<Photo>> {
        val query = SimpleSQLiteQuery(
            "SELECT * FROM ${Photo.TABLE_NAME} WHERE ${Photo.COL_IMPORTED_AT} > $since AND ${Photo.COL_DELETED_AT} IS NULL ORDER BY ${sort.field.columnName} ${sort.order.sql}"
        )
        return observeAll(query)
    }
